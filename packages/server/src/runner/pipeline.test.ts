...
describe("run planning", () => {
  ...
  it("maps career stages without accidentally applying during discovery", () => {
    expect(planCareer("career", "discover")).toEqual({ onboardOnly: null, discover: true, apply: false, target: null, rotate: false });
    expect(planCareer("all", "onboard:12")).toEqual({ onboardOnly: 12, discover: false, apply: false, target: null, rotate: false });
