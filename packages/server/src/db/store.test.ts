  ...
  it("are idempotent on a file db and enable WAL", () => {
    ...
    try {
      ...
      const names = a.db
        ...
        .all()
        .map((r) => r.name);
      expect(names).toEqual(["001_init.sql", "002_hh_resumes_created_at.sql", "003_company_limiter.sql", "004c_chat_interview.sql", "005d_interview_outcome.sql", "006_study.sql", "007a_agent.sql", "007b_kb.sql", "007c_kb_reviews.sql"]);
