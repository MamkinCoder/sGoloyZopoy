...
export interface TelegramCommands {
  chatIds: string[];
  /** `args` = the text after the command word ("/salary go" → "go"). */
      ...
      try {
        ...
        for (const u of updates) {
          ...
          if (cmds && m?.text?.startsWith("/") && cmds.chatIds.includes(String(m.chat.id))) {
            ...
            const reply = await cmds.onCommand(word.split("@")[0]!.toLowerCase(), m.text.slice(word.length).trim()).catch((e: unknown) => `ошибка: ${e instanceof Error ? e.message : String(e)}`);
