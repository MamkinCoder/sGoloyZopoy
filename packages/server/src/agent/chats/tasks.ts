...
import { OPEN_TASK_STATES, type ChatMessage, type ChatTask, type ChatThread, type ChatTopic, type KbBrief, type Profile, type TapReply, type Vacancy } from "@sgz/shared";
...
export function reconcileThread(env: ChatEnv, thread: ChatThread, target: string, choices: string[] = [], superseded?: ChatTask): ChatTask | null {
  ...
  if (!open) {
    // A task that failed over these same messages is not retried every sync (the queue alerted once);
...
/** Ends an open task without a reply and marks its messages handled. */
...
/** A job of this task gave up: the task shows as failed in the panel (the queue already alerted). */
export function failTask(env: ChatEnv, taskId: number, error: string): void {
  env.store.moveChatTask(taskId, OPEN_TASK_STATES as ChatTask["state"][], "failed", { lastError: error }, iso(env));
}
...
export async function reviewTask(env: ChatEnv, taskId: number): Promise<void> {
  ...
  const tgMessageId = await env.review.ask({ ...task, topics }, false);
  env.store.patchChatTask(task.id, "awaiting_review", { tgMessageId }, iso(env));
  env.enqueue("chats.remind", { taskId: task.id }, { key: `remind:${task.id}`, runAfter: later(env, REMIND_AFTER_MS) });
  env.enqueue("chats.fallback", { taskId: task.id }, { key: `fallback:${task.id}`, runAfter: later(env, FALLBACK_AFTER_MS) });
...
export async function remindTask(env: ChatEnv, taskId: number): Promise<void> {
  ...
  const tgMessageId = await env.review.ask(task, true);
...
export async function draftTask(env: ChatEnv, taskId: number): Promise<void> {
  ...
  if (unknown.length) {
    ...
    if (env.store.moveChatTask(task.id, "drafting", "awaiting_review", { topics }, iso(env))) env.enqueue("chats.review", { taskId: task.id }, { key: `review:${task.id}` });
    return;
  }
  if (reply.unknown_skills?.length) throw new Error(`answer_chat still asks about answered skills: ${reply.unknown_skills.join(", ")}`);
  ...
  if (reply.needs_human) {
    ...
    if (thread.state !== "invited") env.store.upsertChatThread({ ...thread, state: "needs_human" });
...
/** Our reply is on the page after the task's last employer message. */
export function repliedOnPage(page: PageMessage[], stored: ChatMessage[], task: ChatTask, draft: string): boolean {
  ...
  return page.slice(from + 1).some((m) => m.direction === "out" && norm(m.text) === norm(draft));
}
...
export async function sendTask(env: ChatEnv, taskId: number): Promise<void> {
  ...
  let page = await read();
  if (!repliedOnPage(page.messages, env.store.listChatMessages(thread.id), task, task.draft)) {
    ...
    page = await read();
    if (!repliedOnPage(page.messages, env.store.listChatMessages(thread.id), task, task.draft)) throw new Error("отправленный ответ не виден в чате");
  }
  ...
  env.store.insertChatMessages(thread.id, page.messages);
  env.store.markAnswered(task.messageIds);
  env.store.moveChatTask(task.id, "sending", "sent", {}, iso(env));
